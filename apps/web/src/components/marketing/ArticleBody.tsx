    ...
    <div className="article-body article-shell">
      {blocks.map((block) => {
        ...
        switch (block.type) {
          ...
          case "h2":
            return (
              <h2 key={key} id={slugifyHeading(block.text)} className="reveal-up">
                {block.text}
              </h2>
            );
          case "h3":
            return (
              <h3 key={key} id={slugifyHeading(block.text)} className="reveal-up">
                {block.text}
              </h3>
            );
          case "h4":
            return (
              <h4 key={key} id={slugifyHeading(block.text)} className="reveal-up">
                {block.text}
              </h4>
            );
          case "h5":
            return (
              <h5 key={key} id={slugifyHeading(block.text)} className="reveal-up">
                {block.text}
              </h5>
            );
          case "ul":
