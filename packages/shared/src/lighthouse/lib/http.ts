...
function assertSafeOutboundUrl(input: RequestInfo | URL): void {
  ...
  if (process.env.ALLOW_PRIVATE_EGRESS !== "true" && isPrivateHost(url.hostname)) {
...
function isAllowedOrigin(origin: string): boolean {
  ...
  if (
    url.protocol === "https:" &&
    (url.hostname === "designedbyanthony.com" || url.hostname.endsWith(".designedbyanthony.com"))
  ) {
    return true;
  }
 ...
 */
export function checkLocalRateLimit(key: string, limit: number, windowMs: number): number | null {
  const now = Date.now();
