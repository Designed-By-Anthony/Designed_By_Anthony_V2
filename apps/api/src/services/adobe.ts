import { generatePdfWithAdobe, buildAdobePayload } from "@dba/shared/lighthouse/lib/adobePdf";
...
export async function generateAuditPdf(
    ...
    try {
        ...
        // Build Adobe payload
        const payload = buildAdobePayload(
            auditData,
            reportId,
            clientEmail,
            leadName,
        ...
        const domainSlug = targetUrl.replace(/^https?:\/\//, '').replace(/\//g, '-').replace(/\./g, '-');
        ...
        console.error(`Adobe PDF generation failed for job ${jobId}:`, error);
        throw error;
    }
}