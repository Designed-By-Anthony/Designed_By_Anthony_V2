...
async function scanRobotsTxt(baseUrl: string): Promise<RobotsTxtResult> {
  try {
    ...
    if (contentType.includes("html")) return { ...DEFAULT_ROBOTS, exists: false };
...
async function scanSitemap(baseUrl: string, robotsSitemapUrls: string[]): Promise<SitemapResult> {
  const urlsToTry = [
    ...robotsSitemapUrls,
...
async function scanRedirectChain(url: string): Promise<RedirectChainResult> {
  ...
  const hasMixedContent = hops.some((h, i) => {
    if (i <= 0) return false;
    const prev = hops[i - 1];
    return prev !== undefined && h.url.startsWith("http://") && prev.url.startsWith("https://");
  });
