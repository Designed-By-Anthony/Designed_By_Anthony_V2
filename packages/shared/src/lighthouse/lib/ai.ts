...
${params.bodyText}`;
  ...
  for (let attempt = 0; attempt < modelsToTry.length; attempt++) {
    ...
    try {
      ...
      if (!parsed.executiveSummary || typeof parsed.conversionScore !== "number") {
        continue;
      }
      ...
      const _label = isFallback ? "fallback" : isRetry ? "retry" : "primary";
...
export function buildFallbackInsight(params: {
  ...
  if (performanceScore >= 90) strengths.push(`Strong ${performanceScore}/100 performance score`);
  if (seoScore >= 90)
    strengths.push(`${seoScore}/100 SEO score — Google can read your site clearly`);
  if (accessibilityScore >= 90)
    strengths.push(`${accessibilityScore}/100 accessibility — inclusive and well-built`);
  if (params.hasLocalBusinessSchema)
    strengths.push("LocalBusiness schema markup in place for local search");
  if (params.hasTelLink) strengths.push("Click-to-call link present for mobile visitors");
  if (params.sitemapExists)
    strengths.push("XML sitemap found — helps search engines discover all pages");
  if (params.hasCanonical) strengths.push("Canonical tag set correctly");
  if (params.metaTitleLength && params.metaTitleLength >= 30 && params.metaTitleLength <= 60)
  ...
  if (seoScore < 90)
    weaknesses.push(`SEO score of ${seoScore} — missing meta tags or heading structure`);
  if (!params.hasForms && !params.hasTelLink)
    weaknesses.push("No contact form or phone link — visitors have no easy way to reach you");
  if (params.missingRecommendedSchema?.length)
    weaknesses.push(`Missing ${params.missingRecommendedSchema.join(" and ")} schema markup`);
  if (params.readabilityScore != null && params.readabilityScore < 45)
    weaknesses.push("Homepage copy is harder to read than ideal for a general audience");
  if (params.wordCount != null && params.wordCount < 200)
    weaknesses.push(`Only ${params.wordCount} words on the homepage — too thin for search engines`);
  while (weaknesses.length > 3) weaknesses.pop();
  while (weaknesses.length < 3) weaknesses.push("Additional optimization opportunities identified");
  ...
  if (actions.length === 0) {
    ...
    actions.push({
      priority: 2,
      action: "Review and improve meta description for better click-through from search results",
      impact: "medium",
      effort: "low",
    });
    actions.push({
      priority: 3,
      action: "Ensure a clear call-to-action is visible above the fold on every page",
      impact: "high",
      effort: "low",
    });
  }
  ...
  if (params.readabilityScore != null && params.readabilityScore >= 60) conversionScore += 5;
 ...
 */
export async function generatePsiArchitecturalSummary(psiJsonSnippet: string): Promise<string> {
  const trimmed = psiJsonSnippet.slice(0, 100_000);
  const ai = createAiClient();
  const model = process.env.GEMINI_DETONATOR_MODEL?.trim() || FALLBACK_GEMINI_MODEL;
  ...
  try {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), DETONATOR_SUMMARY_TIMEOUT_MS);
    const result = await ai.models.generateContent({
      model,
