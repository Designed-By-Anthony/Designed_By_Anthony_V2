...
import { div as MotionDiv, li as MotionLi, ul as MotionUl } from "framer-motion/client";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { useCallback, useState } from "react";
import { CARD_HAS_TEXT_PAD, SURFACE_CARD_TECHNICAL } from "@/design-system/sections";
...
const LABEL_STYLE =
  "text-[10px] font-bold uppercase tracking-[0.2em] text-[rgb(var(--accent-bronze-rgb)/0.85)] mb-2";
const HEADING_STYLE =
  "font-[family-name:var(--font-display)] text-2xl font-medium tracking-[-0.035em] text-brand-charcoal mb-4";
    ...
    >
      <div className="mb-12 grid gap-8 lg:grid-cols-[1fr_360px] items-start">
        ...
        <div className={`${SURFACE_CARD_TECHNICAL} ${CARD_HAS_TEXT_PAD} bg-white/[0.02]`}>
          <p className="text-[11px] font-bold uppercase text-brand-charcoal/30 mb-2">
            Deliverables
          </p>
          <div className="space-y-3">
            {/* Primary CTA — book a strategy session */}
            <Link href="/contact" className="btn-premium-primary w-full block text-center">
      ...
      >
        {allRings.map((s) => (
          ...
          >
            <ScoreRing score={s.val} label={s.lab} countDelay={prefersReduced ? 0 : s.delay} />
          </MotionLi>
        ))}
      </MotionUl>
      ...
      <div className="grid gap-10 lg:grid-cols-12">
        <div className="lg:col-span-8 space-y-10">
          ...
          <section className="text-bubble is-bordered">
            ...
            <ExecutiveSummaryBody
              text={data.aiInsight?.executiveSummary || "No summary available."}
            />
          </section>
          ...
          <section className={`${SURFACE_CARD_TECHNICAL} ${CARD_HAS_TEXT_PAD}`}>
            ...
            <div className="grid gap-4">
              {data.aiInsight?.prioritizedActions?.slice(0, 5).map((item, i) => (
                <div
                  key={i}
                  className="flex items-start gap-5 p-5 rounded-2xl bg-white/[0.03] border border-white/5 transition-colors hover:bg-white/[0.05]"
                >
                  ...
                  <div>
                    <p className="text-[16px] font-semibold text-brand-charcoal/90 mb-1">
                      {item.action}
                    </p>
                    <div className="flex gap-2">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-brand-charcoal/30">
                        {item.impact} Impact
                      </span>
        ...
        <aside className="lg:col-span-4 space-y-10">
            ...
            <div className="space-y-5">
              {[
                ...
                >
                  ...
                  <span className="text-sm font-bold text-[rgb(var(--accent-bronze-rgb))]">
                    {m.v || "—"}
                  </span>
                </div>
              ))}
          ...
          <div
            className={`${SURFACE_CARD_TECHNICAL} ${CARD_HAS_TEXT_PAD} bg-[rgb(var(--accent-bronze-rgb)/0.03)]`}
          >
