 ...
 */
export function estimateIndexCoverage(sitemap: SitemapResult, moz: MozMetrics): IndexCheckResult {
  const sitemapCount = sitemap.exists && sitemap.urlCount > 0 ? sitemap.urlCount : null;
  const mozCount =
    moz.found && moz.pagesCrawled != null && moz.pagesCrawled > 0 ? moz.pagesCrawled : null;
