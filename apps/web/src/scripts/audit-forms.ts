...
function readDefaultLeadEndpoint(): string {
  ...
  if (typeof document !== "undefined") {
    const fromDom = document.documentElement?.dataset?.leadWebhook?.trim() ?? "";
    if (fromDom) return fromDom;
  }
  const fromEnv =
    typeof process !== "undefined" && typeof process.env?.NEXT_PUBLIC_LEAD_WEBHOOK_URL === "string"
...
function resolveFormEndpoint(rawEndpoint: string | null | undefined): string {
  ...
  try {
    ...
    const isDbaApi = url.hostname === DBA_API_HOST && url.pathname.startsWith("/api/");
...
function trackAuditFormStart(form: HTMLFormElement): void {
  ...
  const pageContext =
    form.querySelector<HTMLInputElement>('input[name="page_context"]')?.value || "unknown";
  const ctaSource =
    form.querySelector<HTMLInputElement>('input[name="cta_source"]')?.value || "unknown";
  const leadSource = form.querySelector<HTMLInputElement>('input[name="lead_source"]')?.value;
  const pageTitle =
    form.querySelector<HTMLInputElement>('input[name="page_title"]')?.value || document.title;
  const pageUrl =
    form.querySelector<HTMLInputElement>('input[name="page_url"]')?.value || window.location.href;
  ...
  if (pageContext === "facebook_offer_landing") {
    pushAnalyticsEvent("facebook_offer_form_start", {
      ...
      source_page: window.location.pathname,
      offer_type:
        form.querySelector<HTMLInputElement>('input[name="offer_type"]')?.value || "unknown",
    });
  }
}
...
function handleAuditSuccess(form: HTMLFormElement): void {
  ...
  const pageTitle =
    form.querySelector<HTMLInputElement>('input[name="page_title"]')?.value || document.title;
  const gaClientId =
    form.querySelector<HTMLInputElement>('input[name="ga_client_id"]')?.value || undefined;
  const pageUrl =
    form.querySelector<HTMLInputElement>('input[name="page_url"]')?.value || window.location.href;
...
async function finalizeAuditFormSubmission(form: HTMLFormElement): Promise<void> {
  const submitButton = form.querySelector<HTMLButtonElement>("[data-form-submit]");
  ...
  try {
    ...
    const data = (await response.json().catch(() => ({}))) as AuditFormResponse & {
      error?: string;
    };
...
function determineLeadSource(pathname: string): string {
  ...
  if (pathname === "/lighthouse" || pathname.startsWith("/lighthouse/")) return "audit_page";
