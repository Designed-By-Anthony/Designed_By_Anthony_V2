...
const ENRICHED_JSONLD_SEGMENTS = new Set(["about", "pricing", "ouredge", "faq", "service-areas"]);
...
export function MarketingJsonLd({ path }: { path: string[] }) {
  ...
  const pathname = path.length === 0 ? "/" : `/${path.filter(Boolean).join("/")}`;
  ...
  if (path.length === 1 && a !== undefined && Object.hasOwn(staticMarketingPageCopy, a)) {
    const copy = staticMarketingPageCopy[a as keyof typeof staticMarketingPageCopy];
    pushWebPageBreadcrumb(copy.title, copy.description);
    return <>{blocks}</>;
  }
