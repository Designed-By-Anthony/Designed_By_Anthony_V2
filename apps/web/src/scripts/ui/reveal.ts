...
function scheduleReveal(
  ...
  const delay = Math.min(Math.max(indexInList, 0), MAX_STAGGER_INDEX) * staggerMs;
  window.setTimeout(() => {
...
export function initRevealAnimations(): void {
  const directionalElements = Array.from(
    document.querySelectorAll<HTMLElement>(DIRECTIONAL_REVEAL_CLASSES.map((c) => `.${c}`).join(","))
  );
    ...
    (entries) => {
      for (const entry of entries) {
        ...
        if (isDirectionalReveal(el)) {
          ...
          scheduleReveal(el, "reveal-active", idx === -1 ? 0 : idx, STAGGER_REVEAL_UP_MS);
