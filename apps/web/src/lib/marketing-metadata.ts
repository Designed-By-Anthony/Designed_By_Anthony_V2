...
import { getServiceAreaLocation, isServiceAreaSlug } from "@/data/serviceAreaLocations";
...
export function resolveMarketingMetadata(path: string[]): Metadata {
  ...
  if (a === "blog" && b && path.length === 2) {
    ...
    const absImage = post.image.startsWith("http") ? post.image : `${SITE}${post.image}`;
    return {
      ...base,
      title: post.title,
  ...
  if (a === "portfolio" && b && path.length === 2) {
    ...
    const absImage = item.image.startsWith("http") ? item.image : `${SITE}${item.image}`;
    return {
      ...base,
      title: item.name,
  ...
  if (path.length === 1 && a !== undefined && Object.hasOwn(staticMarketingPageCopy, a)) {
    const staticCopy = staticMarketingPageCopy[a as keyof typeof staticMarketingPageCopy];
    return {
      ...base,
