...
import { btnOutline, btnPrimary, btnSecondaryProof } from "@/design-system/buttons";
  ...
  return (
    <MarketingChrome footerCta={homeFooterCta}>
      ...
      <section className="section-shell section-shell--wash">
        <div className="section-container">
          <MotionStagger as="ul" className="marketing-link-grid" staggerDelay={0.08}>
      ...
      {longform.map((section) => (
        <section key={section.heading} className="section-shell section-shell--longform">
          <MotionReveal className="section-container" y={16} duration={0.6}>
              ...
              <div className="text-bubble is-bordered">
                {section.paragraphs.map((p, i) => (
                  <p
                    ...
                    className={
                      i > 0
                        ? "mt-4 text-[0.95rem] text-brand-charcoal/75"
                        : "text-[0.95rem] text-brand-charcoal/75"
                    }
                  >
                    {p}
                  </p>
...
function ServiceDetailPage({ slug }: { slug: string }) {
  const service = MARKETING_SERVICES.find((s) => s.path === `/services/${slug}`);
  if (!service) notFound();
  ...
  return (
    <MarketingChrome footerCta={homeFooterCta}>
      <PageHero kind="services" title={service.name} subtitle={service.description} />
      <ProseBlock
        paragraphs={[
...
function BlogIndex() {
  return (
    <MarketingChrome footerCta={homeFooterCta}>
      ...
      />
      <section className="section-shell">
        <MotionStagger className="section-container blog-index-grid" staggerDelay={0.08}>
            ...
            {articleBlocks?.length ? (
              ...
              <div className="marketing-prose">
                ...
                <p className="reveal-up">
                  Full editorial for this URL is being expanded in the Next.js site. If you need the
                  prior version on short notice, email{" "}
...
function PortfolioIndex() {
  return (
    <MarketingChrome footerCta={homeFooterCta}>
      ...
      <section className="section-shell">
        <MotionStagger className="section-container featured-work-grid" staggerDelay={0.08}>
...
function PortfolioCaseStudy({ slug }: { slug: string }) {
  ...
  return (
    <MarketingChrome footerCta={homeFooterCta}>
      <PageHero kind="portfolio" title={item.name} subtitle={item.description} />
      <section className="section-shell">
        <MotionReveal className="section-container marketing-prose reveal-up" y={16} duration={0.6}>
          ...
          <div className="section-container">
            <div className="contact-form-shell surface-card reveal-up">
              ...
              <p className="contact-form-shell__lede">
                Tell us what you are trying to fix — we reply within one business day.
              </p>
              <SovereignLeadForm />
            </div>
          </div>
...
function ThankYouPage() {
  ...
  return (
    <MarketingChrome footerCta={homeFooterCta}>
      <PageHero kind="thank-you" title={base.title} subtitle={base.description} />
      <section className="section-shell">
        <MotionReveal className="section-container marketing-prose" y={16} duration={0.6}>
...
export function MarketingSiteRouter({ path }: { path: string[] }) {
  ...
  if (path.length === 1 && a !== undefined && Object.hasOwn(staticMarketingPageCopy, a)) {
