...
export function initGbpRoiCalculator(): void {
  document.querySelectorAll<HTMLElement>("[data-gbp-roi-calculator]").forEach((root) => {
    ...
    form.addEventListener("submit", (event: Event) => {
      ...
      const additionalRevenue = Math.round(additionalCustomersPerDay * value * 30);
      const roi = Math.round(((additionalRevenue - cost) / cost) * 100);
      const customersDayDisplay = formatCustomersPerDay(additionalCustomersPerDay);
