...
export const adminRoute = new Elysia({ prefix: "/api/admin" }).post(
	...
	async ({ request, body, set, store }) => {
		...
		if (secret) {
			...
			console.warn(
				"[admin/promote] ADMIN_PROMOTE_SECRET is unset — refusing promote in production-like safety.",
			);
			set.status = 503;
		...
		const vaultUrl =
			env?.VAULT_PUBLIC_URL?.trim() || "https://designedbyanthony.com/vault";

		if (!isResendConfigured()) {
			console.error("[admin/promote] RESEND_API_KEY missing — vault synced but no email");
			set.status = 500;
			return {
				error:
					"Vault access synced, but transactional email is not configured.",
			};
		}
