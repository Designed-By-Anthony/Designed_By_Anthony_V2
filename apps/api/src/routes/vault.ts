...
async function resolveVaultClient(
	db: ReturnType<typeof createD1Client>,
	accessEmailLower: string,
) {
...
export const vaultRoute = new Elysia({ prefix: "/api/vault" })
	...
	.post(
		...
		async ({ request, body, set, store }) => {
			...
			const clientLabel =
				client.company_name?.trim() || client.email || "Client";
			...
			if (!isResendConfigured()) {
				console.error("[vault/message] RESEND_API_KEY missing after insert");
				set.status = 500;
				return {
					error:
						"Message was saved but notifications are not configured. Contact support.",
				};
			}
			...
			const subject =
				summaryLine.length > 180 ? `${summaryLine.slice(0, 177)}…` : summaryLine;
