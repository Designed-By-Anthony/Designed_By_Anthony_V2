...
function toAuditData(json: Record<string, unknown>): AuditData {
  ...
  return {
    ...
    indexCoverage: isObject(json.indexCoverage)
      ...
      : undefined,
    places: isObject(json.places) ? (json.places as unknown as AuditData["places"]) : undefined,
...
export default function LighthouseReportViewerPage() {
        ...
        <main className={SECTION_SHELL_TECHNICAL}>
          <div className={`${SECTION_CONTAINER} max-w-2xl`}>
            <div className={`${SURFACE_CARD_TECHNICAL} ${CARD_HAS_TEXT_PAD} text-center`}>
              ...
              <Link
                href="/lighthouse"
                className="inline-flex rounded-xl bg-white/5 border border-white/10 px-8 py-3 text-sm font-bold text-brand-charcoal hover:bg-white/10"
              >
                Back to Scanner
              </Link>
            </div>
  ...
  return (
    <MarketingChrome>
      <main className={SECTION_SHELL_TECHNICAL}>
        <div className={SECTION_CONTAINER}>
          <AuditResults data={data} reportId={id} onReset={() => router.push("/lighthouse")} />
        </div>
      </main>
    </MarketingChrome>
  );
