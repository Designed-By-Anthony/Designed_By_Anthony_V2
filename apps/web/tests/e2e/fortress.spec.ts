...
test.describe("Sovereign Fortress — web + admin", () => {
	...
	test("conversion flow — pSEO audit form submits (loading or terminal outcome)", async ({ page }) => {
		...
		await expect(
			page.getByText("Submitting...").or(page.getByText("Thanks — we received your audit request.")).or(
				page.getByText("Failed to submit form"),
			),
