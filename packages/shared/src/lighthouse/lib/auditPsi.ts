...
async function readJsonBody(response: Response): Promise<{ data: unknown; parseOk: boolean }> {
  try {
...
> {
  const settled = await Promise.allSettled([fetchPageSpeedRunPagespeed(targetUrl, apiKey)]);
