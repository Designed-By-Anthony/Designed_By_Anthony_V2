  ...
  containers.forEach((container) => {
    ...
    container.addEventListener(
      ...
      (e) => {
        ...
        const siblings = container.querySelectorAll<HTMLDetailsElement>("details");
        siblings.forEach((d) => {
...
export function initFaqAccordion(): void {
  ...
  const accordions = Array.from(document.querySelectorAll<HTMLElement>("[data-faq-accordion]"));

  accordions.forEach((accordion) => {
    if (accordion.dataset.faqBound === "true") return;
    accordion.dataset.faqBound = "true";
    const triggers = Array.from(accordion.querySelectorAll<HTMLElement>(".faq-trigger"));
    ...
    triggers.forEach((trigger) => {
      trigger.addEventListener("click", () => {
        ...
        triggers.forEach((otherTrigger) => {
          ...
          const otherPanel = otherParent?.querySelector<HTMLElement>(".faq-panel");
