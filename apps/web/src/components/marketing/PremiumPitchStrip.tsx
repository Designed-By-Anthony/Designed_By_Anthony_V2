...
const SECTION = "py-[var(--section-space-tight)] border-y border-brand-border bg-brand-linen";
...
const GRID = "list-none m-0 p-0 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4";
const CARD =
...
const LABEL = "text-[0.68rem] font-extrabold tracking-[0.2em] uppercase text-brand-accent";
const CARD_TITLE = "text-[1.08rem] font-extrabold tracking-[-0.03em] m-0 leading-[1.25]";
const CARD_BODY = "m-0 text-[0.9rem] leading-[1.55] text-brand-charcoal/80 flex-1";

export function PremiumPitchStrip({ variant = "home" }: { variant?: "home" | "services" }) {
  const items =
    variant === "home"
      ? [
          {
  ...
  return (
    <section className={SECTION} aria-labelledby={`premium-pitch-heading-${variant}`}>
